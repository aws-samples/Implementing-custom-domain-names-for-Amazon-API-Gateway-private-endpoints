...
EOF
              ...
              remote)
                ...
                read -rp "Bucket Region [${current_region:-${AWS_REGION}}]: " bucket_region
                bucket_region=${bucket_region:-${current_region:-${AWS_REGION}}}
                ...
                if echo "${config_ddb}" | grep -E "^(y|Y).*$" ; then
                    ...
                    if ! [ "$(aws dynamodb describe-table --table-name "${ddb_table}" --query "Table.KeySchema[?KeyType=='HASH'].AttributeName" --output text 2>/dev/null)" == "LockID" ]; then
                        echo "Invalid dynamodb table configuration, please see https://developer.hashicorp.com/terraform/language/settings/backends/s3 for details."
                        exit 1
                    fi
                fi
                cat << EOF > "${backend_file}"
...
echo > "${tmp_var_file}"
sed -E 's/^(.*VARIABLES:.*| *| *- *|.*---.*)//g;/^$/d;s/: */=/g' "${var_file}" | while read -r line
do
...
EOF
            ...
            pushd "${src_dir}/terraform" > /dev/null
                ...
                tfvars_file="${src_dir}/outputs/terraform/${APP_NAME}-${APP_ENVIRONMENT}.tfvars"
                echo > "${tfvars_file}"
                sed -E 's/^(.*VARIABLES:.*| *| *- *|.*---.*)//g;/^$/d;s/: */=/g' "${var_file}" | while read -r line
                do
                  if [[ $line =~ ^EXTERNAL_PRIVATE_SUBNETS_ID.? ]]; then
